...
\begin{adjustwidth}{0.5cm}{}
    ...
    \label{norm1}
    The function CKDpriv(($k_{par}$, $c_{par}$), i) $\rightarrow$ ($k_i$, $c_i$) computes a normal child extended private key from the parent extended key.
    ...
    The function CKDpub(($K_{par}$, $c_{par}$), i) $\rightarrow$ ($K_i$, $c_i$) computes a child extended public key from the parent extended key. It is only defined for non-hardened child keys.
    ...
    The function CKDpriv(($k_{par}$, $c_{par}$), i) → ($k_i$, $c_i$) computes a child extended private key from the parent extended key.
...
\begin{enumerate}
    ...
    \item The returned chain code ci is $I_R$ and returned child key ki is $parse_{256}(I_L)$.
          \begin{quote}
...
\begin{adjustwidth}{0.5cm}{}
    ...
    Let $k$ = ($k_L$, $k_R$) be the extended private key and $K$ the public key. Extended private child key $k_i$ = ($k_L $, $k_R$) for child i is produced as \autoref{fig:slip23_private}.
    \begin{figure}[ht!]
        \centering
    ...
    \begin{enumerate}
        ...
        \item Check whether i $\geq$ $2^{31}$ (whether the child is a hardened key).
              \begin{itemize}
                  \item If so (hardened child):
                        \begin{quote}
                            Return ci = HMAC-SHA512(Key = $c_{par}$, Data = 0x01 $\parallel$ $k_{par}$ $\parallel$ $ser_{32}$(i)).
                        \end{quote}
                  \item If not (normal child):
                        \begin{quote}
                            Return ci = HMAC-SHA512(Key = $c_{par}$, Data = 0x03 $\parallel$ $ser_P(K_{par})$ $\parallel$ $ser_{32}$(i)).
                        \end{quote}
              \end{itemize}
              \bigskip
    ...
    \begin{enumerate}
        ...
        \item Calculate the child public key $K_i$:
              \begin{quote}
                  $K_i$ = Kp + 8$Z_L$B. If $K_i$ equals to the identity point (0,1) return false.
              \end{quote}
    ...
    The authors gives us an working derivation schema but they didn't explained why did they choose spe$c_i$fic metrics for the schema. For example they shortened to 28-byte of $Z_L$ for the child keys.
